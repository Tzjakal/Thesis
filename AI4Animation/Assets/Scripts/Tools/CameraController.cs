...
public class CameraController : MonoBehaviour {
	...
	public enum MODE {Follow, LookAt, FreeView, FixedView, FirstPerson}
	...
	void Update() {
        ...
        //{
        //    if(GameObject.FindGameObjectWithTag("target").transform.position.y > 0.5)
        //        Target = GameObject.FindGameObjectWithTag("target").transform;
        //    else
        //        Target = GameObject.FindGameObjectWithTag("wolf").transform;
        //}
        //else
        //    Target = GameObject.FindGameObjectWithTag("adam").transform;
    ...
    private void UpdateFirstPerson()
    {
        ...
        TargetPosition = GameObject.FindGameObjectWithTag("adam").transform.position;
        TargetPosition.y += 1.8f;
        transform.rotation = GameObject.FindGameObjectWithTag("adam").transform.rotation;

        if (Input.GetKey(KeyCode.UpArrow))
        {
            transform.RotateAround(TargetPosition, transform.right, -1);
        }
        if (Input.GetKey(KeyCode.DownArrow))
        {
            transform.RotateAround(TargetPosition, transform.right, 1);
        ...
        if (Input.GetKey(KeyCode.LeftArrow))
        {
            transform.RotateAround(TargetPosition, Vector3.up, -1);
        }
        if (Input.GetKey(KeyCode.RightArrow))
        {
            transform.RotateAround(TargetPosition, Vector3.up, 1);
	...
	void OnGUI() {
        ...
        GUI.backgroundColor = Mode == MODE.FirstPerson ? UltiDraw.Mustard : UltiDraw.Black;
        if (GUI.Button(Utility.GetGUIRect(0.85f, 0.25f, 0.1f, 0.04f), "First Person", GetButtonStyle()))
        {
            SetMode(MODE.FirstPerson);
        }
        GUI.color = Color.black;
		FOV = GUI.HorizontalSlider(Utility.GetGUIRect(0.85f, 0.30f, 0.1f, 0.025f), FOV, 0f, 10f, GetSliderStyle(), GetThumbStyle());
